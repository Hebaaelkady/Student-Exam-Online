         ...
         function checkAnswers() {
		  ...
		  function roundReloaded(num, dec) {
			...
			function judgeSkills(score) {
				var returnString;
					if (score==100) returnString = "ممتاز اجابات رائعة"

					else if (score>90) returnString = "جيد جدا بالتركيز ستصل للامتياز"

					else if (score>70) returnString = "جيد حاول تحسين اجاباتك"

					else if (score>50) returnString = "مقبول"
					
					else if (score>35) returnString = "ضعيف حاول مذاكرة الدرس مرة اخري"

					else if (score>20) returnString = "ضعيف جدا يرجو المتابعة بتركيز!"
					
					else returnString = "للاسف راسب حاول مرة اخري"
            ...
            $('.btnShowResult').click(function(){
                ...
                var results = checkAnswers(), 
					 		  resultSet = '',
							  trueCount = 0,
							  answerKey = ' الاجابات <br />',
							  score; 
                for (var i = 0, ii = results.length; i < ii; i++){
                    if (results[i] == true) trueCount++;
                    resultSet += '<div class="resultRow"> السؤال رقم ' + (i + 1) + (results[i]== true ? "<div class='correct'><span>صحيح</span></div>": "<div class='wrong'><span>خطأ</span></div>") + "</div>";
                ...
                resultSet = '<h2 class="qTitle">' +judgeSkills(score) + '<br/> درجتك هي '+score+'%</h2>' + resultSet + answerKey;
